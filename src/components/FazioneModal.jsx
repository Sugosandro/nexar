...
export default function FazioneModal({ initialData = null, onSave, onClose }) {
  ...
  const suggestions  = memQ ? chars.filter(e => !members.includes(e.id) && e.name.toLowerCase().includes(memQ.toLowerCase())).slice(0, 6) : [];
      ...
      <div className="modal">
        ...
        <div className="fg"><label className="fl">Membri</label>
          <div onClick={() => document.getElementById('fmemInput').focus()}
            ...
            {members.map(id => { const el = chars.find(c => c.id === id); if (!el) return null;
            ...
            <div style={{ position: 'relative', flex: 1, minWidth: 120 }}>
              <input id="fmemInput" type="text" placeholder={members.length ? '' : 'Cerca personaggio…'} value={memQ}
